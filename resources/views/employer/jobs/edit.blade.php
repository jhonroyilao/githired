<x-dashboard-shell title="Create Job Listing">
    ...
    <div class="max-w-5xl">
        ...
        <form method="POST" action="{{ route('employer.jobs.update', $job) }}" class="mt-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-5 md:grid-cols-2">
                ...
                <div>
                    ...
                    <select
                        ...
                        <option value="remote" @selected(old('location_type', $job->location_type) === 'remote')>
                            Remote
                        </option>

                        <option value="onsite" @selected(old('location_type', $job->location_type) === 'onsite')>
                            On-site
                        </option>

                        <option value="hybrid" @selected(old('location_type', $job->location_type) === 'hybrid')>
                            Hybrid
                        </option>
                    </select>
                ...
                <div>
                    ...
                    <select
                        ...
                        @foreach (\App\Enums\JobType::cases() as $type)
                            <option
                                value="{{ $type->value }}"
                               @selected(old('type', $job->type) === $type->value)>
                ...
                <div>
                    ...
                    <select
                        ...
                        <option value="entry" @selected(old('experience_level', $job->experience_level) === 'entry')>
                            Entry
                        </option>

                        <option value="mid" @selected(old('experience_level', $job->experience_level) === 'mid')>
                            Mid
                        </option>

                        <option value="senior" @selected(old('experience_level', $job->experience_level) === 'senior')>
                            Senior
                        </option>
                    </select>
