...
class ResumeUploadTest extends TestCase
{
    ...
    public function test_applicant_can_upload_a_pdf_resume(): void
    {
        ...
        $response = $this->actingAs($user)->post(route('applicant.resume.store'), ['resume' => $file]);
    ...
    public function test_guests_cannot_upload_a_resume(): void
    {
        ...
        $response = $this->post(route('applicant.resume.store'), ['resume' => $file]);
    ...
    public function test_uploading_a_new_resume_supersedes_the_previous_current_resume(): void
    {
        ...
        $this->actingAs($user)->post(route('applicant.resume.store'), ['resume' => $file]);
        ...
        $this->assertSame(1, ResumeDocument::where('user_id', $user->id)->where('is_current', true)->count());
    }

    public function test_uploading_a_resume_does_not_affect_other_users_current_resume(): void
    {
        $userA = User::factory()->create(['role' => UserRole::Applicant->value]);
        $userB = User::factory()->create(['role' => UserRole::Applicant->value]);
        ...
        $this->actingAs($userA)->post(route('applicant.resume.store'), ['resume' => $file]);
    ...
    {
        $owner = User::factory()->create(['role' => UserRole::Applicant->value]);
        $intruder = User::factory()->create(['role' => UserRole::Applicant->value]);
